...
TEmuVt102::TEmuVt102(int lines, int columns)
{
  screen[0] = new TEScreen(lines,columns);
  screen[1] = new TEScreen(lines,columns);
  scr = screen[0];
  print_mode = false;
  initTokenizer();
  reset();
//xaudit = fopen("xaudit","w");
}
...
void TEmuVt102::setScreen(int n)
{
  scr = screen[n&1];
}
...
void TEmuVt102::setColumns(int columns)
{
...
void TEmuVt102::changeSize(int columns, int lines)
{
  screen[0]->resizeImage(lines,columns);
  screen[1]->resizeImage(lines,columns);
}

int TEmuVt102::lines() const
{
  return scr->getLines();
}

int TEmuVt102::columns() const
{
  return scr->getColumns();
}

void TEmuVt102::sndBlock(const char* txt,int len)
{
...
/*
   ...
   - DCS        - Device control string    <ESC>'P' {Text} <ESC>'\´
...
#define Xte        (Xpe                        &&     cc           ==  7 )
#define Dce        (ppos>=2  && pbuf[1] == 'P'                           )
#define Dee        (Dce                        &&     cc           =='\\')
#define ces(C)     (            cc < 256 &&    (tbl[  cc  ] & (C)) == (C) && !Xte && !Dce)
...
if (xaudit) { fprintf(xaudit,"%c",cc); fflush(xaudit); } //FIXME 'fwrite' here, UNICODE
  ...
  if (getMode(MODE_Ansi)) // decide on proper action
  {
    ...
    if (Dee         ) { scan_buffer_report();                   resetToken(); return; }
    if (Dce         ) {                                                       return; }
...
//FIXME: I've seen a DCS (Device Control String) with xterm, lately  - ESC P ... ESC backslash 
...
//printf("Set Title, Set Icon - %d \"%.*s\"\n",arg,ppos-i-2,pbuf+i+1);
#if 0
...
//printf("tau(%d,%d,%d, %d,%d)\n",(token>>0)&0xff,(token>>8)&0xff,(token>>16)&0xffff,p,q);
  // printer handling CSI 5 i, CSI 4i ...
  switch (token)
  {
    case TY_CSI_PS('i',    5) : 
      print_mode = true; 
  ...
  switch (token)
  {
    ...
    case TY_CTL___('G'      ) : /* BEL: ignored                      */ break; //VT100
...
void TEmuVt102::resetModes()
{
  ...
  resetMode(MODE_NewLine  ); saveMode(MODE_NewLine);
    setMode(MODE_Ansi     ); saveMode(MODE_Ansi);
  //
  resetMode(MODE_Insert   ); saveMode(MODE_Insert);
    setMode(MODE_Cursor   ); saveMode(MODE_Cursor);
  //
  holdScreen = false;
}
...
void TEmuVt102::setMode(int m)
{
  ...
  switch (m)
  {
    case MODE_AppScreen : setScreen(1);
    break;
  }
...
void TEmuVt102::resetMode(int m)
{
  ...
  switch (m)
  {
    case MODE_AppScreen : setScreen(0);
    break;
  }
  if (m < MODES_SCREEN || m == MODE_NewLine)
  {
...
BOOL TEmuVt102::getMode(int m) const
{
...
// added for restore feature ///////////////////////////////////////////////

const TEScreen* TEmuVt102::getScreen(int n) const
{
  return screen[n&1];
}
